...
function cloud_prepare() {
	...
	if(empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
		return error('-1', "您的程序需要在微动力云服务平台注册你的站点资料, 来接入云平台服务后才能使用相应功能.");
	}
	return true;
}
...
function cloud_m_prepare($name) {
	...
	$pars['module'] = $name;
	$dat = cloud_request('http://addons.wdlcms.com/gateway.php', $pars);
	if (is_error($dat)) {
		return $dat;
	}
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://addons.wdlcms.com/gateway.php', $pars, $headers, 300);
	if(is_error($dat)) {
