...
class Xhw_voiceModuleSite extends WeModuleSite {
	...
	public function doWebpost(){
		...
		$setting = pdo_fetch("SELECT * FROM ".tablename(xhw_voice_setting)." WHERE weid = '{$_W['uniacid']}' ORDER BY id DESC LIMIT 1");
                                    $qrcode = $this->createWebUrl("mp3qrcode",
                                        array('id'=>intval($_GPC['id']),
        ...
        public function doWebMp3Qrcode(){
             global $_W,$_GPC;
               require IA_ROOT."/framework/library/qrcode/phpqrcode.php";
                   $data = $_W['siteroot'].$this->createMobileUrl("makemp3",
                    array('id'=>intval($_GPC['id']),
                        "access_token"=>$_GPC['access_token'],
                        "appid"=>$_GPC['appid'],
                        "appsecret"=>$_GPC['appsecret'],
		...
		$setting = pdo_fetch("SELECT * FROM ".tablename(xhw_voice_setting)." WHERE weid = '{$_W['uniacid']}' ORDER BY id DESC LIMIT 1");
                  $access_token = $_GPC['access_token'];
                    ...
                    $accesskey = $_GPC['accesskey'];
                    $secretkey = $_GPC['secretkey'];
                    if(!empty($setting['mp3'])){
                        die('已经生成过MP3, 不需要重新生成!');
                    }
