...
class Amouse_HouseModuleSite extends WeModuleSite
{
    public function doMobileIndex()
    {
        ...
        $list    = pdo_fetchall('SELECT * FROM ' . tablename('amouse_house') . " $condition ORDER BY createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
    ...
    public function doMobileNew()
    {
        ...
        if ($a == 'rent') {
            if ($_W['ispost']) {
                ...
                message('提交房产信息成功，请等待审核!', $this->createMobileUrl('index', array(
                    'type' => $_GPC['type']
                )), 'success');
            }
    ...
    public function doMobileDetail()
    {
        ...
        if ($a == 'rent') {
            $item = pdo_fetch("SELECT * FROM " . tablename('amouse_house') . " WHERE weid=:weid AND id = :id", array(
                ':weid' => $weid,
                ':id' => $id
            ));
            if (empty($item)) {
            ...
            if ($type == 0 || $type == 1) {
                $recommeds = pdo_fetchall("SELECT * FROM " . tablename('amouse_house') . " WHERE weid=:weid AND (type='0' OR type='1') AND recommed=1 ORDER BY createtime ", array(
                    ':weid' => $weid
                ));
                ...
                $recommeds = pdo_fetchall("SELECT * FROM " . tablename('amouse_house') . " WHERE weid=:weid AND (type='2' OR type='3') AND recommed=1 ORDER BY createtime ", array(
                    ':weid' => $weid
                ));
                $url       = $_W['siteroot'] . "app/" . substr($this->createMobileUrl('detail', array(
                    'a' => 'rent',
            ...
            $item = pdo_fetch("SELECT * FROM " . tablename('amouse_newflats') . " WHERE weid=:weid AND id = :id", array(
                ':weid' => $weid,
                ':id' => $id
            ));
            if (empty($item)) {
    ...
    public function doMobileList()
    {
        ...
        $list      = pdo_fetchall('SELECT * FROM ' . tablename('amouse_newflats') . " $condition ORDER BY createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
    ...
    public function doMobilePosition()
    {
        ...
        if ($op == 'positionsort') {
            ...
            $psize      = 20;
            $type       = $_GPC['type'];
            $list       = pdo_fetchall("SELECT * FROM " . tablename('amouse_house') . " WHERE weid ='{$weid}' AND location_c = '" . $location_c . "' AND status=1 ORDER BY createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
    ...
    public function doMobileUpdateCount()
    {
        ...
        $result['state'] = 0;
        $result['msg']   = ' ';
        $detail          = pdo_fetch("SELECT * FROM " . tablename('amouse_newflats') . " WHERE `id`=:id", array(
            ':id' => $id
        ));
        if ($detail) {
            if ($op == "read") {
                ...
                pdo_update('amouse_newflats', $data, array(
                    'id' => $id
                ));
            } elseif ($op == "like") {
                $data2 = array(
                ...
                pdo_update('amouse_newflats', $data2, array(
                    'id' => $id
                ));
            }
            $result['msg']   = ' ';
    ...
    public function doWebHouse()
    {
        ...
        if ($op == 'display') {
            ...
            $id = intval($_GPC['id']);
            if ($id > 0) {
                if (pdo_update('amouse_house', array(
                    'status' => 1
                ), array(
                    'id' => $id
                )) === false) {
                ...
                message('审核成功！', $this->createWebUrl('house', array(
                    'op' => 'display',
                    'name' => 'amouse_house'
                )), 'success');
            }
            ...
            if ($id > 0) {
                pdo_update('amouse_house', array(
                    'recommed' => $recommed
                ), array(
                    'id' => $id
                ));
                message($msg . '成功！', $this->createWebUrl('house', array(
                    'op' => 'display',
                    'name' => 'amouse_house'
                )), 'success');
            }
            ...
            $id   = intval($_GPC['id']);
            $temp = pdo_delete("amouse_house", array(
                'id' => $id
            ));
            message('删除数据成功！', $this->createWebUrl('house', array(
                'op' => 'display',
                'name' => 'amouse_house'
            )), 'success');
        } elseif ($op == 'post') {
            ...
            load()->func('tpl');
            if ($id > 0) {
                $item = pdo_fetch('SELECT * FROM ' . tablename('amouse_house') . " WHERE id=:id", array(
                    ':id' => $id
                ));
            }
            if (checksubmit('submit')) {
                ...
                if (empty($id)) {
                    ...
                    pdo_update('amouse_house', $data, array(
                        'id' => $id
                    ));
                }
                message('更新新楼盘数据成功！', $this->createWebUrl('house', array(
                    'op' => 'display',
                    'name' => 'amouse_house'
                )), 'success');
            }
    ...
    public function doWebPremises()
    {
        ...
        if ($op == 'display') {
            ...
            load()->func('tpl');
            if ($id > 0) {
                $item = pdo_fetch('SELECT * FROM ' . tablename('amouse_newflats') . " WHERE id=:id", array(
                    ':id' => $id
                ));
            }
            if (checksubmit('submit')) {
                ...
                $insert = array(
                    ...
                    'discounted_costs' => $_GPC['discounted_costs'],
                    'payment' => $_GPC['payment'],
                    'business' => $_GPC['business'],
                    'banks' => $_GPC['banks'],
                    'trading_area' => $_GPC['trading_area'],
                    'park' => $_GPC['park'],
                    'hotel' => $_GPC['hotel'],
                    'supermarket' => $_GPC['supermarket'],
                    'humanities' => $_GPC['humanities'],
                    'supporting' => $_GPC['supporting'],
                    'internal' => $_GPC['internal'],
                    'parking_number' => $_GPC['parking_number'],
                    'base' => $_GPC['base'],
                    'equally' => $_GPC['equally'],
                    'surrounding' => $_GPC['surrounding'],
                    'landscape' => $_GPC['landscape'],
                    'hospitals' => $_GPC['hospitals'],
                    'school' => $_GPC['school'],
                ...
                if (empty($id)) {
                    ...
                    pdo_update('amouse_newflats', $insert, array(
                        'id' => $id
                    ));
                }
                message('更新新楼盘数据成功！', $this->createWebUrl('premises', array(
                    'op' => 'display',
                    'name' => 'amouse_house'
                )), 'success');
            }
            ...
            $id   = intval($_GPC['id']);
            $temp = pdo_delete("amouse_newflats", array(
                'id' => $id
            ));
            message('删除数据成功！', $this->createWebUrl('premises', array(
                'op' => 'display',
                'name' => 'amouse_house'
            )), 'success');
        }
    ...
    public function doWebSlide()
    {
        ...
        if (checksubmit('submit')) {
            ...
            if (!empty($_GPC['attachment'])) {
                foreach ($_GPC['attachment'] as $index => $row) {
                    ...
                    pdo_update('amouse_house_slide', $data, array(
                        'id' => $index
                    ));
                }
            }
        ...
        if ($op == 'delete') {
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                $item = pdo_fetch("SELECT * FROM " . tablename('amouse_house_slide') . " WHERE id = :id", array(
                    ':id' => $id
                ));
                if (empty($item)) {
                    message('图片不存在或是已经被删除！');
                }
                pdo_delete('amouse_house_slide', array(
                    'id' => $item['id']
                ));
            } else {
        ...
        $photos = pdo_fetchall("SELECT * FROM " . tablename('amouse_house_slide') . " WHERE weid = :weid ORDER BY listorder DESC", array(
            ':weid' => $weid
        ));
    ...
    {
        global $_GPC, $_W;
        return pdo_fetch('SELECT * FROM ' . tablename('amouse_sysset') . ' WHERE weid=:weid limit 1', array(
            ':weid' => $weid
        ));
    }
    public function doWebSysset()
    {
        ...
        if (checksubmit('submit')) {
            ...
            if (!empty($set)) {
                pdo_update('amouse_sysset', $data, array(
                    'id' => $set['id']
                ));
            } else {
